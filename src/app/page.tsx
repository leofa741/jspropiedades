...
import { FaSearch, FaBuilding, FaMapMarkerAlt, FaStar, FaFilter, FaArrowRight } from 'react-icons/fa';
...
// ─────────────────────────────────────────────────────────────
// 🔹 Componentes de Loading LOCAL (solo para secciones dinámicas)
// ─────────────────────────────────────────────────────────────

function PropertiesSectionLoader() {
  return (
...
function CategoriesSectionLoader() {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="animate-pulse p-5 rounded-2xl bg-slate-800/50 border border-slate-700/50">
          <div className="flex items-center justify-between mb-3">
            <div className="w-8 h-8 bg-slate-700/50 rounded-full" />
            <div className="w-10 h-5 bg-slate-700/50 rounded-full" />
          </div>
...
// ─────────────────────────────────────────────────────────────
// 🔹 Componente Principal - SIN Suspense global
// ─────────────────────────────────────────────────────────────
...
function PageContent() {
  // ✅ Estados LOCALES por sección dinámica (no bloquean el render inicial)
  ...
  const [neighborhoods, setNeighborhoods] = useState<Array<{ slug: string; name: string; count: number; ciudad: string }>>([]);
  ...
  // Efecto para el mouse move del background (estático, no bloquea)
  useEffect(() => {
  ...
  // 📥 Cargar datos - CADA SECCIÓN TIENE SU PROPIO LOADING
  useEffect(() => {
    const fetchData = async () => {
      // 1. Propiedades destacadas (sección independiente)
      try {
        const res = await fetch('/api/gestion/public/propiedades?destacado=true&limit=6');
        if (res.ok) {
          const data: ApiResponse = await res.json();
          setFeaturedProperties(data.propiedades);
        }
      } catch (err) {
      ...
      // 2. Todas las propiedades (sección independiente)
      try {
        const res = await fetch('/api/gestion/public/propiedades?limit=12');
        if (res.ok) {
          const data: ApiResponse = await res.json();
      ...
      // 3. Categorías y barrios (sección independiente)
      try {
        const res = await fetch('/api/gestion/public/categorias');
        if (res.ok) {
          const cats: CategoriesResponse = await res.json();
          setCategories(cats.categorias);
          setNeighborhoods(cats.barrios);
        }
      } catch (err) {
  ...
  // ─────────────────────────────────────────────────────────────
  // 🔹 Componente reutilizable de beneficio (estático)
  // ─────────────────────────────────────────────────────────────
  const BenefitCard = ({
    icon,
    title,
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="relative w-full left-0 right-0">
        ...
        >
          <div className="max-w-7xl mx-auto px-6 pt-40 sm:pt-48">
            ...
            <div className="absolute bottom-85 left-0 right-0 pb-40 sm:pb-32 px-4 opacity-80">
      ...
      {/* ═══════════════════════════════════════════════════════
          SECCIÓN BENEFICIOS - ESTÁTICA (se muestra inmediatamente)
          ═══════════════════════════════════════════════════════ */}
      <section className="relative py-16 sm:py-20 lg:py-28 overflow-hidden bg-slate-950">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 animate-fadeInUp">
          ...
          <div className="text-center mt-12 sm:mt-16 animate-fadeInUp" style={{ animationDelay: '600ms' }}>
            <a
              href="/contact"
      ...
      {/* ═══════════════════════════════════════════════════════
          SECCIÓN VALOR - ESTÁTICA (se muestra inmediatamente)
          ═══════════════════════════════════════════════════════ */}
      <section className="relative -mt-16 sm:-mt-20 py-20 sm:py-24 lg:py-28 overflow-hidden bg-slate-950">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-0 bg-gradient-to-b from-slate-900 via-slate-950 to-slate-950" />
          ...
          <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-16">
            <div className="lg:w-1/2 space-y-6 sm:space-y-8 animate-fadeInUp" style={{ animationDelay: '100ms' }}>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2">
                ...
                <a
                  href="/contact"
                  className="group relative inline-flex items-center justify-center gap-2.5 px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl font-semibold text-sm tracking-wide transition-all duration-300 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-purple-500/40 text-white backdrop-blur-sm active:scale-[0.98]"
                >
            ...
            {/* Card de propiedad destacada */}
            <div className="lg:w-1/2 flex justify-center lg:justify-end animate-fadeInUp" style={{ animationDelay: '300ms' }}>
              <div className="relative group w-full max-w-md">
                <div className="absolute -inset-2 sm:-inset-4 bg-gradient-to-r from-cyan-500/30 via-purple-500/30 to-pink-500/30 rounded-3xl blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" aria-hidden="true" />
                <div className="absolute -inset-0.5 bg-gradient-to-r from-cyan-400 via-purple-500 to-pink-500 rounded-2xl opacity-30 group-hover:opacity-60 transition-opacity duration-500 blur-sm pointer-events-none" aria-hidden="true" />
               
                <div className="relative bg-slate-900/80 backdrop-blur-xl rounded-2xl p-2 sm:p-3 border border-white/10 shadow-2xl shadow-purple-900/30 group-hover:shadow-purple-900/50 transition-all duration-500 group-hover:scale-[1.01]">
                  ...
                  <div className="flex items-center justify-between px-3 py-2 mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-slate-500 tracking-wider ml-2 font-mono">
                        propiedades-destacadas
                      </span>
                    </div>
                    {/* Badge de exclusiva */}
                    <span className="hidden sm:flex items-center gap-1 px-2 py-0.5 rounded-full bg-gradient-to-r from-amber-500/20 to-yellow-500/20 border border-amber-500/30">
                      <span className="w-1 h-1 rounded-full bg-amber-400 animate-pulse" />
                      <span className="text-[9px] font-semibold text-amber-300 tracking-wider uppercase">Exclusiva</span>
                    </span>
                  </div>

                  {/* Imagen principal */}
                  <div className="relative overflow-hidden rounded-xl aspect-[4/3]">
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/40 to-transparent z-10 pointer-events-none" aria-hidden="true" />
                    <Image
                      src={featuredProperties[0]?.imagen || '/img/logo-dorado-removebg.png'}
                      alt={featuredProperties[0]?.titulo || 'Propiedad exclusiva'}
                      ...
                      <div className="absolute bottom-3 left-3 right-3 z-20 flex items-end justify-between gap-2">
                        <div className="px-3 py-2 rounded-lg bg-slate-900/80 backdrop-blur-md border border-white/10">
                          <span className="text-sm font-bold text-white">
                            {formatPrice(featuredProperties[0].precio.monto, featuredProperties[0].precio.moneda, featuredProperties[0].precio.tipo)}
                          </span>
                          <span className="text-[10px] text-slate-400 block mt-0.5">
                            {featuredProperties[0].ubicacion.barrio}
                          </span>
                        </div>
                        
                        {/* Botón ver detalles */}
                        <button 
                          onClick={() => setSelectedProperty(featuredProperties[0])}
                          className="group/btn flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 transition-all duration-300"
                        >
                          <span className="text-[10px] font-semibold text-white tracking-wider uppercase">Detalles</span>
                  ...
                  {/* Contenido inferior - SIN descripción larga */}
                  <div className="mt-4 pt-4 border-t border-white/10">
                    {/* Título truncado */}
                    <h3 className="text-sm font-bold text-white line-clamp-1 mb-3 group-hover:text-amber-300 transition-colors duration-300">
                      {featuredProperties[0]?.titulo || 'Propiedad destacada'}
                    </h3>
                    
                    {/* Grid de features con iconos - Reemplaza la descripción */}
                    <div className="grid grid-cols-3 gap-2">
                      <div className="flex flex-col items-center gap-1 p-2 rounded-lg bg-white/5 border border-white/5 hover:bg-white/10 transition-colors duration-300">
                        ...
                        <span className="text-[10px] font-semibold text-white">
                          {featuredProperties[0]?.caracteristicas?.dormitorios || 3}
                        </span>
                        <span className="text-[9px] text-slate-500 uppercase tracking-wider">Dorms</span>
                      </div>
                      ...
                      <div className="flex flex-col items-center gap-1 p-2 rounded-lg bg-white/5 border border-white/5 hover:bg-white/10 transition-colors duration-300">
                        ...
                        <span className="text-[10px] font-semibold text-white">
                          {featuredProperties[0]?.caracteristicas?.banios || 2}
                        </span>
                        <span className="text-[9px] text-slate-500 uppercase tracking-wider">Baños</span>
                      </div>
                      ...
                      <div className="flex flex-col items-center gap-1 p-2 rounded-lg bg-white/5 border border-white/5 hover:bg-white/10 transition-colors duration-300">
                        ...
                        <span className="text-[10px] font-semibold text-white">
                          {featuredProperties[0]?.caracteristicas?.metrosCubiertos || 120}m²
                        </span>
                        <span className="text-[9px] text-slate-500 uppercase tracking-wider">Superf.</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute -bottom-4 -left-4 w-12 h-12 bg-gradient-to-br from-purple-400/20 to-pink-500/20 rounded-xl blur-lg animate-pulse pointer-events-none" style={{ animationDelay: '1s' }} aria-hidden="true" />
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ═══════════════════════════════════════════════════════
          🏆 PROPIEDADES DESTACADAS - DINÁMICA (con loading local)
          ═══════════════════════════════════════════════════════ */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8 sm:mb-10">
            <div>
              ...
              <p className="text-slate-400 mt-2 text-sm sm:text-base">Las mejores oportunidades seleccionadas para vos</p>
            </div>
            <Link
              href="/propiedades?destacado=true"
              className="text-violet-400 hover:text-violet-300 font-medium flex items-center gap-2 text-sm sm:text-base"
            >
              Ver todas <FaArrowRight className="w-4 h-4" />
            </Link>
          </div>

          {/* ✅ Loading LOCAL solo para esta sección */}
          {featuredLoading ? (
            <PropertiesSectionLoader />
          ) : featuredProperties.length === 0 ? (
            <div className="text-center py-12 text-slate-500">
              <FaBuilding className="text-4xl mb-3 mx-auto opacity-50" />
              <p>No hay propiedades destacadas en este momento</p>
              <Link href="/propiedades" className="text-violet-400 hover:text-violet-300 mt-4 inline-block text-sm">
      ...
      {/* ═══════════════════════════════════════════════════════
          📂 EXPLORAR POR CATEGORÍA - DINÁMICA (con loading local)
          ═══════════════════════════════════════════════════════ */}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                {categories.map((cat) => (
                  <Link
                    ...
                    className="group p-4 sm:p-5 rounded-2xl bg-slate-900/80 border border-slate-700/50 hover:border-violet-500/40 hover:bg-slate-800/90 transition-all duration-300"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-xl sm:text-2xl">{getTipoIcon(cat.slug)}</span>
                      <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-violet-500/20 text-violet-400 text-[10px] sm:text-xs font-medium">
                        {cat.count}
                      </span>
                    </div>
                    <h3 className="text-white font-semibold group-hover:text-violet-400 transition-colors text-sm sm:text-base">
                      {cat.name}
                    </h3>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {neighborhoods.map((barrio) => (
                      <Link
                        ...
                        className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-slate-800/60 border border-slate-700/50 text-slate-300 hover:bg-violet-500/20 hover:border-violet-500/40 hover:text-violet-400 transition-all text-xs sm:text-sm flex items-center gap-1.5"
                      >
                        {barrio.name}
                        <span className="px-1.5 sm:px-2 py-0.5 rounded-full bg-slate-700 text-slate-400 text-[10px] sm:text-xs">
                          {barrio.count}
                        </span>
                      </Link>
                    ))}
      ...
      {/* ═══════════════════════════════════════════════════════
          🏠 LISTADO GENERAL - DINÁMICA (con loading local)
          ═══════════════════════════════════════════════════════ */}
      <section className="py-12 sm:py-16 bg-slate-900/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 sm:mb-10">
            <div>
              ...
              <p className="text-slate-400 mt-2 text-sm">
                {propertiesLoading ? 'Cargando...' : `${filteredProperties.length} propiedad${filteredProperties.length !== 1 ? 'es' : ''} disponible${filteredProperties.length !== 1 ? 's' : ''}`}
              </p>
            </div>
            <Link
              href="/propiedades"
              className="inline-flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-medium transition-all shadow-lg shadow-violet-900/30 text-sm"
            >
          ...
          {propertiesLoading ? (
            ...
            <div className="text-center py-12 text-slate-500">
              ...
              <p>{searchQuery ? `No se encontraron propiedades para "${searchQuery}"` : 'No hay propiedades disponibles'}</p>
              {searchQuery && (
                <button
          ...
          <div className="text-center mt-8 sm:mt-10">
            <Link
              href="/propiedades"
              className="inline-flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl bg-slate-800/60 border border-slate-700/50 text-slate-300 hover:bg-slate-700/60 hover:border-violet-500/40 hover:text-white transition-all font-medium text-sm"
            >
              Ver más propiedades
      ...
      {/* ═══════════════════════════════════════════════════════
          📞 CTA SECTION - ESTÁTICA (se muestra inmediatamente)
          ═══════════════════════════════════════════════════════ */}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold transition-all shadow-lg shadow-emerald-900/30 text-sm"
            >
              <svg className="w-4 h-4 sm:w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" /></svg>
              Contactar por WhatsApp
            </a>
            <Link
              href="/contact"
              className="inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-4 rounded-xl bg-slate-800/60 border border-slate-700/50 text-slate-300 hover:bg-slate-700/60 hover:border-violet-500/40 hover:text-white transition-all font-semibold text-sm"
            >
              Enviar consulta
            </Link>
          </div>
      ...
      {/* ═══════════════════════════════════════════════════════
          🎯 MODAL DE PROPIEDAD - AL NIVEL RAÍZ
          ═══════════════════════════════════════════════════════ */}
      {selectedProperty && (
        <div 
        ...
        >
          <div 
            className="relative max-w-2xl w-full max-h-[90vh] overflow-y-auto bg-slate-900/95 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl shadow-purple-900/50"
          ...
          >
            ...
            <div className="relative aspect-video">
              <Image
                src={selectedProperty.imagen || '/img/logo-dorado-removebg.png'}
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <p className="text-sm text-slate-400 flex items-center gap-2">
                  ...
                  {selectedProperty.ubicacion.barrio}, {selectedProperty.ubicacion.ciudad}
                </p>
              </div>

              {/* Precio destacado */}
              <div className="p-4 rounded-xl bg-gradient-to-br from-amber-500/10 to-yellow-500/10 border border-amber-500/20">
                <span className="text-xs text-amber-400 uppercase tracking-wider font-semibold">Precio</span>
                <p className="text-3xl font-bold text-white mt-1">
                  {formatPrice(selectedProperty.precio.monto, selectedProperty.precio.moneda, selectedProperty.precio.tipo)}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                  <span className="text-xs text-slate-500 uppercase tracking-wider">Dormitorios</span>
                  <p className="text-lg font-bold text-white mt-1">{selectedProperty.caracteristicas?.dormitorios || '-'}</p>
                </div>
                <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                  <span className="text-xs text-slate-500 uppercase tracking-wider">Baños</span>
                  <p className="text-lg font-bold text-white mt-1">{selectedProperty.caracteristicas?.banios || '-'}</p>
                </div>
                <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                  <span className="text-xs text-slate-500 uppercase tracking-wider">Superficie</span>
                  <p className="text-lg font-bold text-white mt-1">{selectedProperty.caracteristicas?.metrosCubiertos || '-'}m²</p>
                </div>
                <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                  <span className="text-xs text-slate-500 uppercase tracking-wider">Cochera</span>
                  <p className="text-lg font-bold text-white mt-1">{selectedProperty.caracteristicas?.cochera ? 'Sí' : 'No'}</p>
                </div>
              </div>

              {/* CTA */}
              <a
                href={`https://wa.me/5491132538837?text=${encodeURIComponent(`Hola, me interesa la propiedad: ${selectedProperty.titulo}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-500 text-slate-900 font-bold hover:from-amber-400 hover:to-yellow-400 transition-all duration-300 shadow-lg shadow-amber-500/30 inline-flex items-center justify-center gap-2"
              >
